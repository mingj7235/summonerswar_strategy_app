package com.joshua.summonerswar.global.config.jwt;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private void checkLogout (String accessToken) {
        if (logoutAccessTokenRedisRepository.existsById(accessToken)) {
            throw new IllegalArgumentException("이미 로그아운 된 회원");
        }
    }

    private void equalsUsernameFromTokenAndUserDetails (String userDetailsUsername, String toeknUsername) {
        if (!userDetailsUsername.equals(toeknUsername)) {
            throw new IllegalArgumentException("username 이 토큰과 맞지 않음");
        }
    }
